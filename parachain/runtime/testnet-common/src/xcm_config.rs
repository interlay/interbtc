#[macro_export]
macro_rules! testnet_xcm_config { ($mod_config:ident) => {
    use super::*;
    ...
    fn base_tx_in_xcm() -> Balance {
        $mod_config::PARENT_TOKEN_ID.one() / 50_000
    }
        ...
        impl Convert<CurrencyId, Option<MultiLocation>> for CurrencyIdConvert {
            fn convert(id: CurrencyId) -> Option<MultiLocation> {
                match id {
                    ...
                    x => {
                        if x == GetRelayChainCurrencyId::get() {
                            Some(MultiLocation::parent())
                        } else if x == GetNativeCurrencyId::get() || x == GetWrappedCurrencyId::get() {
            ...
            fn convert(location: MultiLocation) -> Option<CurrencyId> {
                fn decode_currency_id(key: Vec<u8>) -> Option<CurrencyId> {
                    ...
                    if let Ok(currency_id) = CurrencyId::decode(&mut &key[..]) {
                        ...
                        match currency_id {
                            $mod_config::WRAPPED_CURRENCY_ID => Some(currency_id),
                            $mod_config::NATIVE_CURRENCY_ID => Some(currency_id),
                            _ => None,
                        }
                    } else {
                        None
                ...
                match location.clone() {
                    x if x == MultiLocation::parent() => Some($mod_config::PARENT_CURRENCY_ID),
                    MultiLocation {
                        parents: 1,
