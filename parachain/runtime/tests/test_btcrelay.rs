...
fn integration_test_submit_block_headers_and_verify_transaction_inclusion() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(
            BTCRelayCall::initialize(parachain_genesis_header, parachain_genesis_height)
                .dispatch(origin_of(account_of(ALICE)))
        );
        for block in test_data.iter().skip(1) {
            assert_ok!(BTCRelayCall::store_block_header(block.get_raw_header())
                .dispatch(origin_of(account_of(ALICE))));
        ...
        for block in test_data.iter() {
            for tx in &block.test_txs {
                ...
                if block.height <= current_height - CONFIRMATIONS {
                    ...
                    assert_err!(
                        BTCRelayCall::verify_transaction_inclusion(
                            txid,
                            block.height,
                            raw_merkle_proof,
                            CONFIRMATIONS,
