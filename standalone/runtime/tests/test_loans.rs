...
fn set_up_market(currency_id: CurrencyId, exchange_rate: FixedU128, ctoken_id: CurrencyId) {
    ...
    assert_ok!(Call::Sudo(SudoCall::sudo {
        call: Box::new(Call::Loans(LoansCall::activate_market {
            asset_id: currency_id,
        })),
    })
    .dispatch(origin_of(account_of(ALICE))));
...
fn test_real_market<R>(execute: impl Fn() -> R) {
    ExtBuilder::build().execute_with(|| {
        ...
        set_up_market(Token(KINT), FixedU128::from_inner(115_942_028_985_507_246_376_810_000), Token(CKINT));
        set_up_market(Token(KSM), FixedU128::from_inner(4_573_498_406_135_805_461_670_000), Token(CKSM));
        execute()
    });
}
...
fn integration_test_issue_expiry_only_parachain_blocks_expired() {
    test_real_market(|| {
        ...
        assert_ok!(
            Call::Loans(LoansCall::collateral_asset {
                asset_id: kint,
                enable: true,
            }).dispatch(origin_of(account_of(USER)))
        );
        ...
        assert_ok!(OraclePallet::_set_exchange_rate(kint, kint_rate.checked_mul(&2.into()).unwrap()));
