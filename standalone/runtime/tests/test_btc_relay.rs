...
fn integration_test_submit_block_headers_and_verify_transaction_inclusion() {
    ExtBuilder::build().execute_without_relay_init(|| {
        ...
        for block in test_data.iter().skip(1) {
            println!("Submitting block height {:?}", block.height);
            ...
            if block.height >= btc_relay::DIFFICULTY_ADJUSTMENT_INTERVAL && block.height % btc_relay::DIFFICULTY_ADJUSTMENT_INTERVAL == 0 {
                assert_ok!(BTCRelayPallet::get_last_retarget_time(CHAIN_ID, block.height));
            }
