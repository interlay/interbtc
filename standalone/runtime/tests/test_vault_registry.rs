...
mod client_release {
    ...
    fn integration_test_vault_registry_set_current_client_release_works() {
        test_with(|_vault_id| {
            ...
            .dispatch(root()));
            assert_eq!(
                VaultRegistryPallet::current_client_release(vault_key),
                Some(vault_release)
            );
            ...
            .dispatch(root()));
            assert_eq!(
                VaultRegistryPallet::current_client_release(oracle_client_name),
                Some(oracle_release)
            );
        });
    }
    ...
    fn integration_test_vault_registry_set_pending_client_release_works() {
        test_with(|_vault_id| {
            ...
            assert_eq!(
                VaultRegistryPallet::pending_client_release(vault_key),
                Some(vault_release)
            );
            assert_eq!(
                VaultRegistryPallet::pending_client_release(oracle_key),
                Some(oracle_release)
            );
        });
    }
