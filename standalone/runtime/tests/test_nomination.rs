...
fn test_with_nomination_enabled_and_vault_opted_in<R>(execute: impl Fn(CurrencyId) -> R) {
    test_with_nomination_enabled(|currency_id| {
        assert_nomination_opt_in(&vault_id_of(VAULT, currency_id));
        execute(currency_id)
    })
}
...
mod spec_based_tests {
    ...
    fn nomination_with_non_running_status_fails(status: StatusCode) {
        ...
        assert_noop!(
            Call::Nomination(NominationCall::deposit_collateral(vault_id.clone(), 100))
                .dispatch(origin_of(account_of(ALICE))),
            SecurityError::ParachainNotRunning,
        );
        assert_noop!(
            Call::Nomination(NominationCall::withdraw_collateral(vault_id.clone(), 100, None))
                .dispatch(origin_of(account_of(ALICE))),
            SecurityError::ParachainNotRunning,
        );
    }
    ...
    fn integration_test_deposit_collateral_preconditions() {
        ...
        test_with(|currency_id| {
            ...
            assert_ok!(
                Call::Nomination(NominationCall::deposit_collateral(vault_id.clone(), DEFAULT_NOMINATION))
                    .dispatch(origin_of(account_of(USER)))
            );
        })
    }
    ...
    fn integration_test_withdraw_collateral_preconditions_collateralization() {
        ...
        test_with_nomination_enabled(|currency_id| {
            ...
            assert_noop!(
                withdraw_nominator_collateral(account_of(USER), &vault_id, default_nomination(currency_id)),
                NominationError::InsufficientCollateral
            );
        });
    }
    ...
    fn integration_test_withdraw_collateral_postconditions() {
        ...
        test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
            ...
            let user_collateral_before_withdrawal =
                NominationPallet::get_nominator_collateral(&vault_id, &account_of(USER)).unwrap();
            withdraw_nominator_collateral(account_of(USER), &vault_id, default_nomination(currency_id)).unwrap();
...
fn integration_test_regular_vaults_are_not_opted_in_to_nomination() {
    test_with_nomination_enabled(|currency_id| {
        ...
        assert_eq!(
            NominationPallet::is_opted_in(&vault_id_of(CAROL, currency_id)).unwrap(),
            false
        );
    })
}
...
fn integration_test_can_nominate_if_opted_in() {
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        ...
        assert_total_nominated_collateral_is(&vault_id, default_nomination(currency_id));
    });
}
    ...
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        ...
        assert_noop!(
            withdraw_vault_collateral(
                &vault_id,
                default_backing_collateral(currency_id).with_amount(|x| x + 1)
            ),
...
fn integration_test_nominated_collateral_cannot_exceed_max_nomination_ratio() {
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        let vault_id = vault_id_of(VAULT, currency_id);
        assert_noop!(
            nominate_collateral(&vault_id, account_of(USER), default_backing_collateral(currency_id)),
            NominationError::DepositViolatesMaxNominationRatio
        );
    });
}
...
fn integration_test_liquidating_a_vault_does_not_force_refund() {
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        let vault_id = vault_id_of(VAULT, currency_id);
        assert_nominate_collateral(&vault_id, account_of(USER), default_nomination(currency_id));
        VaultRegistryPallet::liquidate_vault(&vault_id_of(VAULT, currency_id)).unwrap();
        let nonce: u32 = VaultStakingPallet::nonce(&vault_id_of(VAULT, currency_id));
        assert_eq!(nonce, 0);
    })
}
...
fn integration_test_vault_withdrawal_cannot_exceed_max_nomination_taio() {
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        ...
        assert_noop!(
            withdraw_vault_collateral(&vault_id, Amount::new(10, currency_id)),
...
fn integration_test_rewards_are_preserved_on_collateral_withdrawal() {
    test_with_nomination_enabled_and_vault_opted_in(|currency_id| {
        ...
        let reward_before_nomination_withdrawal =
            VaultStakingPallet::compute_reward(INTERBTC, &vault_id, &account_of(USER)).unwrap();
