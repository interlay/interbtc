...
fn should_distribute_rewards_equally() {
    run_test(|| {
        ...
        assert_ok!(Reward::distribute(DOT, fixed!(100)));
...
fn should_distribute_uneven_rewards_equally() {
    run_test(|| {
        assert_ok!(Reward::deposit_stake(DOT, &ALICE, fixed!(50)));
        assert_ok!(Reward::deposit_stake(DOT, &BOB, fixed!(50)));
        assert_ok!(Reward::distribute(DOT, fixed!(451)));
...
fn should_not_update_previous_rewards() {
    run_test(|| {
        ...
        assert_ok!(Reward::distribute(DOT, fixed!(1000)));
...
fn should_withdraw_reward() {
    run_test(|| {
        ...
        assert_ok!(Reward::distribute(DOT, fixed!(2344)));
...
fn should_withdraw_stake() {
    run_test(|| {
        ...
        assert_ok!(Reward::distribute(DOT, fixed!(4242)));
...
fn should_not_withdraw_stake_if_balance_insufficient() {
    run_test(|| {
        ...
        assert_ok!(Reward::distribute(DOT, fixed!(2000)));
...
fn should_not_distribute_rewards_without_stake() {
    run_test(|| {
        assert_ok!(Reward::distribute(DOT, fixed!(1000)), fixed!(0));
...
fn should_distribute_with_many_rewards() {
    ...
    run_test(|| {
        ...
        for _ in 0..30 {
            ...
            assert_ok!(Reward::distribute(DOT, fixed!(rng.gen::<u32>() as i128)));
        }
