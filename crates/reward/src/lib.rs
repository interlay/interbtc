...
pub mod pallet {
    ...
    #[pallet::metadata(T::AccountId = "AccountId", T::SignedFixedPoint = "SignedFixedPoint")]
    pub enum Event<T: Config<I>, I: 'static = ()> {
        DepositStake(T::AccountId, T::SignedFixedPoint),
        WithdrawStake(T::AccountId, T::SignedFixedPoint),
        WithdrawReward(T::AccountId, T::SignedFixedPoint),
    }
    ...
    pub type TotalRewards<T: Config<I>, I: 'static = ()> = StorageValue<_, SignedFixedPoint<T, I>, ValueQuery>;

    /// Used to compute the rewards for a participant's stake.
    #[pallet::storage]
    #[pallet::getter(fn reward_per_token)]
    pub type RewardPerToken<T: Config<I>, I: 'static = ()> = StorageValue<_, SignedFixedPoint<T, I>, ValueQuery>;

    /// The stake of a participant in this reward pool.
    #[pallet::storage]
    #[pallet::getter(fn stake)]
    pub type Stake<T: Config<I>, I: 'static = ()> =
        StorageMap<_, Blake2_128Concat, T::AccountId, SignedFixedPoint<T, I>, ValueQuery>;
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
    ...
    pub fn get_total_rewards() -> Result<<T::SignedFixedPoint as FixedPointNumber>::Inner, DispatchError> {
        Ok(Self::total_rewards()
            .into_inner()
    ...
    T::SignedFixedPoint: MaybeSerializeDeserialize + Debug,
{
    ...
    fn deposit_stake(account_id: &T::AccountId, amount: Self::SignedFixedPoint) -> Result<(), DispatchError> {
        ...
        Self::deposit_event(Event::<T, I>::DepositStake(account_id.clone(), amount));
        Ok(())
    }
    ...
    fn distribute(reward: Self::SignedFixedPoint) -> Result<Self::SignedFixedPoint, DispatchError> {
        ...
        let reward_div_total_stake = reward
            ...
            .ok_or(Error::<T, I>::ArithmeticUnderflow)?;
        checked_add_mut!(RewardPerToken<T, I>, &reward_div_total_stake);
        checked_add_mut!(TotalRewards<T, I>, &reward);
        Ok(reward)
    }
    ...
    fn withdraw_stake(account_id: &T::AccountId, amount: Self::SignedFixedPoint) -> Result<(), DispatchError> {
        if amount > <Stake<T, I>>::get(&account_id) {
            return Err(Error::<T, I>::InsufficientFunds.into());
        }

        checked_sub_mut!(Stake<T, I>, &account_id, &amount);
        ...
        Self::deposit_event(Event::<T, I>::WithdrawStake(account_id.clone(), amount));
        Ok(())
    }
    ...
    fn withdraw_reward(
        ...
        let reward = Self::compute_reward(account_id)?;
        let reward_as_fixed =
            SignedFixedPoint::<T, I>::checked_from_integer(reward).ok_or(Error::<T, I>::TryIntoIntError)?;
        checked_sub_mut!(TotalRewards<T, I>, &reward_as_fixed);
        ...
        Self::deposit_event(Event::<T, I>::WithdrawReward(account_id.clone(), reward_as_fixed));
        Ok(reward)
    }
}
