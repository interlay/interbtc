...
fn mint_and_deposit(account_id: AccountId, pool_id: CurrencyId, amount: Balance) {
    assert_ok!(Tokens::set_balance(
        RuntimeOrigin::root(),
        account_id,
        pool_id,
    ...
    assert_ok!(Farming::deposit(RuntimeOrigin::signed(account_id), pool_id, amount));
}
...
fn should_overwrite_existing_schedule() {
    run_test(|| {
        ...
        // check pool balance
        assert_eq!(
            Tokens::total_balance(reward_currency, &Farming::pool_account_id(&pool_id)),
            reward_schedule_1.total().unwrap(),
        );

        // add stake so we can distribute
        mint_and_deposit(0, pool_id, 100);
...
fn should_deposit_and_withdraw_stake() {
    run_test(|| {
        ...
        assert_err!(
            Farming::deposit(RuntimeOrigin::signed(account_id), pool_id, pool_tokens),
        ...
        mint_and_deposit(account_id, pool_id, pool_tokens);
        ...
        assert_eq!(Tokens::free_balance(pool_id, &account_id), withdraw_amount);
    })
}
...
fn should_deposit_stake_and_claim_reward() {
    run_test(|| {
        ...
        assert_eq!(
            Tokens::free_balance(reward_currency, &account_id),
