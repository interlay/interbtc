...
fn add_liquidity_when_withdraw_more_than_available_in_meta_pool_should_fail() {
    new_test_ext().execute_with(|| {
        ...
        let meta_pool_id = 1;
        assert_noop!(
            StableAmm::calculate_currency_amount(meta_pool_id, vec![Balance::MAX, 3e18 as Balance], false),
...
fn add_liquidity_with_expected_lp_amount_in_meta_pool_should_success() {
    new_test_ext().execute_with(|| {
        setup_test_meta_pool();
        let meta_pool_id = 1;
        let calculated_pool_token_amount =
            StableAmm::calculate_currency_amount(meta_pool_id, vec![1e18 as Balance, 3e18 as Balance], true).unwrap();
...
fn add_liquidity_when_lp_token_amount_has_small_slippage_in_meta_pool_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let calculated_pool_token_amount =
            StableAmm::calculate_currency_amount(meta_pool_id, vec![1e18 as Balance, 3e18 as Balance], true).unwrap();
        let calculated_pool_token_amount_with_negative_slippage = calculated_pool_token_amount * 999 / 1000;
        let calculated_pool_token_amount_with_positive_slippage = calculated_pool_token_amount * 1001 / 1000;
        assert_ok!(StableAmm::add_liquidity(
            RawOrigin::Signed(BOB).into(),
            meta_pool_id,
            vec![1e18 as Balance, 3e18 as Balance],
...
fn remove_liquidity_in_meta_pool_with_expected_return_amount_underlying_currency_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let second_token_balance_before = get_user_balance(meta_pool_info.currency_ids[1], &CHARLIE);
        ...
        let expected_balances =
            StableAmm::calculate_base_remove_liquidity(&meta_pool_info, pool_token_balance_before).unwrap();
...
fn remove_liquidity_in_meta_pool_when_min_amounts_not_reached_due_to_front_running_should_not_work() {
    new_test_ext().execute_with(|| {
        ...
        let expected_balances = StableAmm::calculate_base_remove_liquidity(&pool, pool_token_balance).unwrap();
        assert_eq!(expected_balances[0], 1498601924450190405);
        assert_eq!(expected_balances[1], 504529314564897436);
...
fn remove_liquidity_imbalance_in_meta_pool_with_max_burn_lp_token_amount_range_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let max_pool_token_amount_to_be_burned =
            StableAmm::calculate_currency_amount(meta_pool_id, vec![1e18 as Balance, 1e16 as Balance], false).unwrap();
...
fn remove_liquidity_one_currency_calculation_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            StableAmm::stable_amm_calculate_remove_liquidity_one_currency(meta_pool_id, 2 * pool_token_balance, 0)
                .unwrap(),
            2999998601797183633
        );
    })
...
fn remove_liquidity_one_currency_calculated_amount_as_min_amount_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let calculated_first_token_amount =
            StableAmm::stable_amm_calculate_remove_liquidity_one_currency(meta_pool_id, pool_token_balance, 0).unwrap();
...
fn swap_with_expected_amounts_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let calculated_swap_return =
            StableAmm::stable_amm_calculate_swap_amount(meta_pool_id, 0, 1, 1e17 as Balance).unwrap();
...
fn set_meta_fee_with_no_admin_should_revert() {
    new_test_ext().execute_with(|| {
        let (_, meta_pool_id) = setup_test_meta_pool();
        assert_noop!(
            StableAmm::set_swap_fee(RawOrigin::Signed(CHARLIE).into(), meta_pool_id, 1e8 as Balance),
            BadOrigin
        );
    })
}
...
fn set_meta_fee_in_limit_should_work() {
    new_test_ext().execute_with(|| {
        let (_, meta_pool_id) = setup_test_meta_pool();
        assert_ok!(StableAmm::set_swap_fee(
            RawOrigin::Root.into(),
            meta_pool_id,
            1e8 as Balance,
        ),);
...
fn set_meta_admin_fee_with_no_admin_should_revert() {
    new_test_ext().execute_with(|| {
        let (_, meta_pool_id) = setup_test_meta_pool();
        assert_noop!(
            StableAmm::set_admin_fee(RawOrigin::Signed(CHARLIE).into(), meta_pool_id, 1e8 as Balance),
            BadOrigin
        );
    })
}
...
fn set_meta_admin_fee_in_limit_should_work() {
    new_test_ext().execute_with(|| {
        let (_, meta_pool_id) = setup_test_meta_pool();
        assert_ok!(StableAmm::set_admin_fee(
            RawOrigin::Root.into(),
            meta_pool_id,
            1e10 as Balance
        ),);
...
fn withdraw_admin_balance_with_zero_admin_fee_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(StableAmm::withdraw_admin_fee(
            RawOrigin::Signed(BOB).into(),
            meta_pool_id
        ));
...
fn withdraw_admin_balance_with_expected_amount_after_swap_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            fee_tokens_balance_after[0] - fee_tokens_balance_before[0],
            1001973776101
        );
...
fn ramp_meta_a_with_non_owner_should_not_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            StableAmm::ramp_a(RawOrigin::Signed(BOB).into(), meta_pool_id, 55, end_timestamp.into()),
            BadOrigin
        );
    })
}
...
fn meta_check_arithmetic_in_swap_should_successfully() {
    new_test_ext().execute_with(|| {
        ...
        let pool_token0_balance = <Test as Config>::MultiCurrency::free_balance(pool.currency_ids[0], &pool.account);
        let pool_token1_balance = <Test as Config>::MultiCurrency::free_balance(pool.currency_ids[1], &pool.account);
