...
impl<T: Config> Pallet<T> {
    ...
    > {
        ...
        Ok((
            BasePool {
                currency_ids: BoundedVec::try_from(currency_ids.to_vec())
                    .map_err(|_| Error::<T>::TooManyPoolBalances)?,
                lp_currency_id,
                token_multipliers: BoundedVec::try_from(rate).map_err(|_| Error::<T>::TooManyPoolBalances)?,
