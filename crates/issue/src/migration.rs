...
pub mod v5 {
    ...
    pub fn migrate_v4_to_v5<T: Config>() -> Weight {
        let mut weight = T::DbWeight::get().reads(1);
        ...
        IssueRequests::<T>::translate(|key, old: v4::DefaultIssueRequest<T>| {
            weight.saturating_accrue(T::DbWeight::get().reads_writes(1, 1));
            log::info!(target: TARGET, "migrating issue #{:?}", &key);
...
mod test {
    ...
    fn migrating_from_v4_to_v5() {
        run_test(|| {
            assert_eq!(crate::StorageVersion::<T>::get(), Version::V4);

            let old = v4::DefaultIssueRequest::<T> {
                requester: 123,
                ...
                griefing_currency: <T as vault_registry::Config>::GetGriefingCollateralCurrencyId::get(),
            };
            let key = H256::zero();
            v4::IssueRequests::<T>::insert(key, old.clone());

            v5::migrate_v4_to_v5::<T>();

            assert_eq!(StorageVersion::get::<Pallet<T>>(), 5);
