...
fn request_issue(
    ...
    Redeem::_request_issue(origin, amount, vault, collateral)
}
...
fn request_issue_ok(
    ...
    match Redeem::_request_issue(origin, amount, vault, collateral) {
        Ok(act) => act,
        Err(err) => {
            println!("{}", err.message());
            panic!(err);
...
fn cancel_issue(origin: AccountId, issue_id: &H256) -> Result<(), Error> {
    Redeem::_cancel_issue(origin, *issue_id)
}
...
fn test_request_issue_insufficient_collateral_fails() {
    run_test(|| {
        Redeem::set_issue_griefing_collateral(10);
