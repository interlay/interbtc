    ...
    pub fn try_from_transaction(transaction: Transaction) -> Result<Self, DispatchError> {
        ...
        for tx in transaction.outputs {
            if let Ok(address) = tx.extract_address() {
                ...
                ensure!(tx.value == 0, Error::<T>::MalformedTransaction);
                // make sure that the op_return is exactly 32 bytes
                ensure!(data.len() == 32, Error::<T>::MalformedTransaction);
                op_returns.push(H256::from_slice(&data));
            } else {
                return Err(Error::<T>::MalformedTransaction.into());
            }
        }
        ...
        match payments.len() {
            ...
            2 => {
                ...
                ensure!(payments[0].1 != payments[1].1, Error::<T>::MalformedTransaction);
            }
            _ => return Err(Error::<T>::MalformedTransaction.into()),
        }
