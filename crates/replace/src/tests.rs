...
fn request_issue(
    ...
    Replace::_request_issue(origin, amount, vault, collateral)
}
...
fn request_replace_ok(
    ...
    match Replace::_request_issue(origin, amount, vault, collateral) {
        Ok(act) => act,
        Err(err) => {
            println!("{}", err.message());
            panic!(err);
...
fn test_request_issue_insufficient_collateral_fails() {
    run_test(|| {
        Replace::set_issue_griefing_collateral(10);
        assert_noop!(
...
#[test]
fn request_replace_invalid_collateral() {
    unimplemented!()
}

#[test]
fn request_replace_invalid_vault_it() {
    unimplemented!()
}

#[test]
fn request_replace_replacing_self() {
    unimplemented!()
}

#[test]
fn request_replace_invalid_block_height() {
    unimplemented!()
}

#[test]
fn request_issue_succeeds() {
    run_test(|| {
        let origin = ALICE;
        let vault = BOB;
        ...
        let request_issue_event = TestEvent::test_events(RawEvent::RequestReplace(
            issue_id,
            origin,
            amount,
            vault,
...
fn test_execute_issue_succeeds() {
    run_test(|| {
        ...
        Replace::set_issue_period(10);
        execute_issue_ok(ALICE, &issue_id);

        let execute_issue_event =
            TestEvent::test_events(RawEvent::ExecuteReplace(issue_id, ALICE, BOB));
        assert!(System::events()
            .iter()
...
fn test_request_replace_succeeds() {
    run_test(|| {
        ...
        let replace_id = request_replace_ok(origin, amount, vault, 0);

        let event = TestEvent::test_events(RawEvent::RequestReplace(
            replace_id,
            origin,
            amount,
            vault,
