...
impl<T: Trait> RichVault<T> {
    ...
    pub fn get_collateral(&self) -> DOTBalance<T> {
        ext::collateral::for_account::<T>(&self.data.id)
    }

    pub fn get_free_collateral(&self) -> Result<DOTBalance<T>> {
        let used_collateral = self.get_used_collateral()?;
        Ok(self.get_collateral() - used_collateral)
    }

    pub fn get_used_collateral(&self) -> Result<DOTBalance<T>> {
    ...
    pub fn issuable_tokens(&self) -> Result<PolkaBTCBalance<T>> {
        // FIXME: figure out how to multiply these two
    ...
    pub fn increase_to_be_issued(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
    ...
    fn force_increase_to_be_issued(&mut self, tokens: PolkaBTCBalance<T>) -> () {
        self.update(|v| v.to_be_issued_tokens += tokens);
    }

    pub fn decrease_to_be_issued(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
        ensure!(
    ...
    pub fn issue_tokens(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
        self.decrease_to_be_issued(tokens)?;
        Ok(self.force_issue_tokens(tokens))
    }

    fn force_issue_tokens(&mut self, tokens: PolkaBTCBalance<T>) -> () {
        self.update(|v| v.issued_tokens += tokens)
    }

    pub fn decrease_issued(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
        let issued_tokens = self.data.issued_tokens;
        ensure!(issued_tokens >= tokens, Error::InsufficientTokensCommitted);
        Ok(self.update(|v| v.issued_tokens -= tokens))
    }

    pub fn increase_to_be_redeemed(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
    ...
    fn force_increase_to_be_redeemed(&mut self, tokens: PolkaBTCBalance<T>) -> () {
        self.update(|v| v.to_be_redeemed_tokens += tokens);
    }

    pub fn decrease_to_be_redeemed(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
    ...
    pub fn decrease_tokens(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
        self.decrease_to_be_redeemed(tokens)?;
        self.decrease_issued(tokens)
    }

    pub fn redeem_tokens(&mut self, tokens: PolkaBTCBalance<T>) -> UnitResult {
        self.decrease_tokens(tokens)
    }

    pub fn transfer(&mut self, other: &mut RichVault<T>, tokens: PolkaBTCBalance<T>) -> UnitResult {
        self.decrease_tokens(tokens)?;
