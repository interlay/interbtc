...
use crate::{Collateral, Config, Error, RichVault, SignedFixedPoint};
use sp_runtime::{
...
fn inner_to_collateral<Collateral: TryFrom<u128>, Inner: FixedPointOperand>(
    x: Inner,
) -> Result<Collateral, SlashingError> {
    let y = UniqueSaturatedInto::<u128>::unique_saturated_into(x);
    TryInto::<Collateral>::try_into(y).map_err(|_| SlashingError::TryIntoIntError)
}

fn collateral_to_inner<Collateral: TryInto<u128>, Inner: FixedPointOperand>(
    x: Collateral,
) -> Result<Inner, SlashingError> {
...
fn collateral_to_fixed<Collateral: TryInto<u128>, SignedFixedPoint: FixedPointNumber>(
    x: Collateral,
) -> Result<SignedFixedPoint, SlashingError> {
    let signed_fixed_point =
        SignedFixedPoint::checked_from_integer(collateral_to_inner::<Collateral, SignedFixedPoint::Inner>(x)?)
            .ok_or(SlashingError::TryIntoIntError)?;
    Ok(signed_fixed_point)
}

pub trait SlashingAccessors<Collateral, SignedFixedPoint, E: From<SlashingError>> {
    ...
    fn get_collateral(&self) -> Collateral;

    fn mut_collateral<F>(&mut self, func: F) -> Result<(), E>
    where
        F: Fn(&mut Collateral) -> Result<(), E>;

    /// Get the total collateral held by a vault, including nomination.
    fn get_total_collateral(&self) -> Result<Collateral, E>;
    ...
    fn get_backing_collateral(&self) -> Result<Collateral, E>;
...
pub trait TryDepositCollateral<
    Collateral: TryInto<u128> + CheckedAdd,
    SignedFixedPoint: FixedPointNumber,
    E: From<SlashingError>,
>: SlashingAccessors<Collateral, SignedFixedPoint, E>
{
    /// Called by the vault or nominator to deposit collateral.
    fn try_deposit_collateral(&mut self, amount: Collateral) -> Result<(), E> {
...
impl<
        T: SlashingAccessors<Collateral, SignedFixedPoint, E>,
        Collateral: TryInto<u128> + CheckedAdd,
        SignedFixedPoint: FixedPointNumber,
        E: From<SlashingError>,
    > TryDepositCollateral<Collateral, SignedFixedPoint, E> for T
{
}

pub trait TryWithdrawCollateral<
    Collateral: TryInto<u128> + TryFrom<u128> + CheckedSub + PartialOrd,
    SignedFixedPoint: FixedPointNumber,
    E: From<SlashingError>,
>: SlashingAccessors<Collateral, SignedFixedPoint, E>
{
    ...
    fn compute_collateral(&self) -> Result<Collateral, E> {
        ...
        let collateral = collateral
            ...
            .ok_or(SlashingError::ArithmeticUnderflow)?;
        inner_to_collateral::<Collateral, SignedFixedPoint::Inner>(
            collateral
                .into_inner()
    ...
    fn try_withdraw_collateral(&mut self, amount: Collateral) -> Result<(), E> {
...
impl<
        ...
        SignedFixedPoint: FixedPointNumber,
        E: From<SlashingError>,
    > TryWithdrawCollateral<Collateral, SignedFixedPoint, E> for T
{
}

impl<T: Config> Slashable<Collateral<T>, SignedFixedPoint<T>, Error<T>> for RichVault<T> {
...
impl<T: Config> SlashingAccessors<Collateral<T>, SignedFixedPoint<T>, Error<T>> for RichVault<T> {
