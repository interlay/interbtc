...
pub mod vault_capacity {
    ...
    pub struct RewardsMigration<Runtime, VaultRewardsInstance, VaultCapacityInstance>(
        sp_std::marker::PhantomData<(Runtime, VaultRewardsInstance, VaultCapacityInstance)>,
    );
    ...
    impl<
        ...
        > OnRuntimeUpgrade for RewardsMigration<Runtime, VaultRewardsInstance, VaultCapacityInstance>
    {
        #[cfg(feature = "try-runtime")]
        fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
            ...
            let prev_count_nonzero = reward::migration::v0::Stake::<Runtime, VaultRewardsInstance>::iter()
                ...
                .count();
            log::info!(target: TARGET, "{prev_count_nonzero} non-zero stake entries will be migrated");
        ...
        fn on_runtime_upgrade() -> Weight {
            ...
            for (vault_id, _) in Vaults::<Runtime>::iter() {
                ...
                log::info!(
                    target: TARGET,
                    "Setting stake to {:?}", expected_stake.amount()
                );
        ...
        fn post_upgrade(state: Vec<u8>) -> Result<(), &'static str> {
            ...
            // log::info!(target: TARGET, "a");
            // // check that totalUserVaultCollateral matches sum of stakes in staking pallet
            ...
            //         .reduce(|a, b| a.saturating_add(b))
            //         .unwrap_or_default();
            //     assert_eq!(amount_from_nominator_stakes, expected_collateral - 2u32.into());
            // }
            log::info!(target: TARGET, "b");
            ...
            for (_nonce, vault_id, total) in staking::TotalCurrentStake::<Runtime>::iter() {
                ...
                let actual_stake = staking::Stake::<Runtime>::iter()
                    .filter_map(
                        |(_nonce, (vault, nominator), _)| {
                            if vault_id == vault {
                                let stake = ext::staking::compute_stake::<Runtime>(&vault, &nominator).unwrap();
                                let fp_stake = staking::Pallet::<Runtime>::compute_stake(&vault, &nominator).unwrap();

                                let xstake = staking::Pallet::<Runtime>::stake_at_index(0u32.into(), &vault, &nominator);
                                let xslash_per_token = staking::Pallet::<Runtime>::slash_per_token_at_index(0u32.into(), &vault);
                                let xslash_tally = staking::Pallet::<Runtime>::slash_tally_at_index(0u32.into(), &vault, &nominator);
                                let xto_slash = staking::Pallet::<Runtime>::compute_amount_to_slash(xstake, xslash_per_token, xslash_tally).unwrap();
                        

                                log::info!(target: TARGET, "stake = {:?}", stake);
                                log::info!(target: TARGET, "fp_stake = {:?}", fp_stake);
                                log::info!(target: TARGET, "x = {:?} {:?} {:?} {:?}", xstake,
                                xslash_per_token,
                                xslash_tally,
                                xto_slash);
                ...
                log::info!(target: TARGET, "expected = {:?}, actual = {:?}", expected.amount(), actual_stake);

                assert!(diff <= 1u32.into());
            }
            log::info!(target: TARGET, "c");
            ...
            log::info!(target: TARGET, "e");

            // check that vault capacity reward stakes match the vaultRewards totalstakes
            for (((), currency), capacity_stake) in reward::Stake::<Runtime, VaultCapacityInstance>::iter() {
                let bitcoin = <Runtime as currency::Config>::GetWrappedCurrencyId::get();
                let total_capacity_amount = Amount::<Runtime>::from_signed_fixed_point(capacity_stake, bitcoin).unwrap();


                let total_reward_stake = ext::reward::total_current_stake::<Runtime>(currency)?;
                let total_reward_amount = total_reward_stake.convert_to(bitcoin)?;

                assert_eq!(total_capacity_amount.amount(), total_reward_amount.amount());
            }
