...
fn preimage_deposit_should_be_reapable_earlier_by_owner() {
    new_test_ext().execute_with(|| {
        ...
        next_block();
        assert_noop!(
            Democracy::reap_preimage(RuntimeOrigin::signed(6), set_balance_proposal_hash(2), u32::MAX),
            Error::<Test>::TooEarly
        );
        next_block();
...
fn preimage_deposit_should_be_reapable() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Democracy::reap_preimage(RuntimeOrigin::signed(5), set_balance_proposal_hash(2), u32::MAX),
